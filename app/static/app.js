...
function badge(status) { const labels = { active: "可用", pending: "待检测", login_pending: "待登录", logging_in: "登录中", login_failed: "登录失败", challenge_required: "需验证", profile_resetting: "重置中", queued: "排队", preparing: "准备", submitted: "已提交", running: "生成中", succeeded: "完成", failed: "失败", expired: "超时", login_required: "需登录", network_error: "网络异常", disabled: "禁用", disabled_low_balance: "低额度" }; return `<span class="badge ${escapeHtml(status)}">${escapeHtml(labels[status] || status || "未知")}</span>`; }
...
function renderAccounts() {
  ...
  $("#accountsBody").innerHTML = items.map((account) => {
    ...
    return `<tr><td><span class="cell-title"><span class="account-id mono">#${account.id}</span>${escapeHtml(account.name)}</span><span class="cell-sub">${escapeHtml(account.email || account.user_id || "")}</span></td>
      ...
      <td><div class="row-actions"><button class="icon-button" data-action="check" data-id="${account.id}" title="检测"><i data-lucide="activity"></i></button><button class="icon-button" data-action="login" data-id="${account.id}" title="CDP 登录"><i data-lucide="monitor-up"></i></button><button class="icon-button" data-action="reset-profile" data-id="${account.id}" title="重置 Profile"><i data-lucide="fingerprint"></i></button><button class="icon-button" data-action="toggle" data-id="${account.id}" data-enabled="${account.enabled}" title="${account.enabled ? "禁用" : "启用"}"><i data-lucide="${account.enabled ? "pause" : "play"}"></i></button>${account.enabled ? "" : `<button class="icon-button danger" data-action="delete" data-id="${account.id}" title="删除"><i data-lucide="trash-2"></i></button>`}</div></td></tr>`;
  }).join("");
...
function defaultPayload() {
  ...
  const payload = { model: model.id || state.selectedModel, prompt: "保持参考主体一致，镜头平稳推进，画面自然连贯", duration: (capabilities.durations || [4])[0], resolution: (capabilities.resolutions || ["480p"])[0], aspect_ratio: (capabilities.aspect_ratios || ["adaptive"])[0], background: true };
...
$("#addAccountButton").addEventListener("click", () => { $("#accountForm").reset(); $("#accountForm").max_concurrency.value = 8; $("#accountForm").use_proxy_pool.checked = true; $("#accountForm").auto_login.checked = true; $("#accountDialog").showModal(); });
